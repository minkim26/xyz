...
async def test_list_parses_packages(manager, fake_subprocess) -> None:
...
async def test_list_returns_empty_on_nonzero_exit(manager, fake_subprocess) -> None:
    with fake_subprocess("xyz.managers.pip", stdout="", returncode=1):
        packages = await manager.list()
    assert packages == []


async def test_list_empty_json_array(manager, fake_subprocess) -> None:
    with fake_subprocess("xyz.managers.pip", stdout="[]"):
        packages = await manager.list()
    assert packages == []


async def test_list_package_has_correct_manager_field(manager, fake_subprocess) -> None:
...
async def test_update_propagates_output(manager, fake_subprocess) -> None:
...
async def test_delete_propagates_output(manager, fake_subprocess) -> None:
...
def test_manager_name_is_pip(manager) -> None:
